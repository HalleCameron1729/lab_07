...
Distribution is skewed as there is more scores in the 4 region, than any other. 

2.  Visualize and describe the relationship between `score` and `bty_avg` using `geom_point()` to represent the data. 

```{r scatterplot}
ggplot(data =evals , mapping = aes(x= score , y = bty_avg)) + geom_point()
ggplot(data =evals, mapping = aes(x = score , y = bty_avg)) + geom_jitter()
```

Seems to move the points to the right, it could be measuring the frequency for the x-axis, less scatter in jitter.
...
tidy(score_bty_fit)
```
Score(hat) = 3.88 + 0.07x bty_avg
On average, the score is 0.07 higher for every bty_avg point. 
...
ggplot(data = evals, mapping = aes(x= bty_avg, y =score)) + geom_smooth(method = "lm", se = FALSE) + geom_jitter()

```

3. Interpret the slope of the linear model in context of the data.

That the score increases as bty_avg increases, however the average stays around score four, and only slightly differs from 4 asthe bty_avg increases.
...
That when the bty_avg is zero, then the score is 3.88, however the average beauty cannot be zero as the range for avg_bty is 1.5-8.5.
...
The linear model explains 3.35% of the variability of score.
